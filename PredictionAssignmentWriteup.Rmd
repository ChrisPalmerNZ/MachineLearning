...
be overfitting, so we can retain fewer variables based on their importance...  
...
predictA_accuracy <- round((length(hitRows) / (length(hitRows) + length(missRows))) * 100, 2)
predictA_accuracy 
```
```{r}
testB$prediction <- predict(rf17final, testB)
hitRows <- which(testB$prediction == testB$classe)
missRows <- which(testB$prediction != testB$classe)
predictB_accuracy <- round((length(hitRows) / (length(hitRows) + length(missRows))) * 100, 2)
predictB_accuracy 
```
...
are very close to the estimated accuracy of 98.79% (test A is less accurate, 
test B is more accurate, their average is `r (predictA_accuracy + predictB_accuracy) / 2`%). 
...
1. Random Forest model that uses 17 variables and 500 trees
2. Estimated out of sample error (OOB) rate is `r rf17oob`%
3. Estimated accuracy of `r 100 - rf17oob`%
4. Application to sampled test data has estimated accuracy of `r (predictA_accuracy + predictB_accuracy) / 2`%
5. Application to supplied test data set had an accuracy of 100%
